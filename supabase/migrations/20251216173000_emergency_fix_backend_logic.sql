-- Emergency Fix to resolve 'calculate_task_due_date_in_month does not exist' error
-- This script explicitly drops the old function and trigger, and re-creates them with the correct logic.

-- 1. Drop existing trigger and functions
DROP TRIGGER IF EXISTS on_work_created_generate_periods ON works;
DROP FUNCTION IF EXISTS public.trigger_auto_generate_periods_and_tasks(); -- Possible previous wrapper name
DROP FUNCTION IF EXISTS public.auto_generate_periods_and_tasks(uuid);

-- 2. Re-create the main logic function (RETURNS void, takes UUID)
...
-- 3. Create Wrapper Trigger Function
CREATE OR REPLACE FUNCTION public.trigger_auto_generate_periods_and_tasks()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  PERFORM public.auto_generate_periods_and_tasks(NEW.id);
  RETURN NEW;
END;
$$;

-- 4. Re-create the trigger using the wrapper
CREATE TRIGGER on_work_created_generate_periods
AFTER INSERT ON works
FOR EACH ROW
EXECUTE FUNCTION public.trigger_auto_generate_periods_and_tasks();
