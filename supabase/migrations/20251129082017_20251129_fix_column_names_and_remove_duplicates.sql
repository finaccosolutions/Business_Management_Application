/*
  # Fix Column Names and Remove Duplicate Functions
  
  ## Issues Fixed
  1. Changed task_period to task_period_type (correct column name)
  2. Removed all duplicate functions
  3. Cleaned up old period generation functions
*/

-- ============================================
-- DROP ALL DUPLICATE AND OLD FUNCTIONS
-- ============================================
...
DROP FUNCTION IF EXISTS auto_generate_next_recurring_period(uuid, date) CASCADE;
DROP FUNCTION IF EXISTS auto_generate_periods_and_tasks(uuid) CASCADE;
...
DROP FUNCTION IF EXISTS trigger_auto_generate_next_recurring_period() CASCADE;
DROP FUNCTION IF EXISTS trigger_auto_generate_periods_for_recurring_work() CASCADE;
...
DROP FUNCTION IF EXISTS generate_period_tasks_for_instance(uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS calculate_last_task_due_date_for_period(uuid, date, date) CASCADE;
...
DROP FUNCTION IF EXISTS calculate_individual_task_period_end_date(uuid, date, date) CASCADE;
DROP FUNCTION IF EXISTS backfill_periods_from_start_date(uuid) CASCADE;
...
DROP FUNCTION IF EXISTS calculate_next_task_period_date(uuid, date) CASCADE;
DROP FUNCTION IF EXISTS calculate_next_task_trigger_date(uuid, date) CASCADE;
DROP FUNCTION IF EXISTS get_all_months_in_period(date, date) CASCADE;
DROP FUNCTION IF EXISTS get_period_boundaries(uuid, date) CASCADE;
DROP FUNCTION IF EXISTS create_recurring_instances_for_work(uuid) CASCADE;
...
BEGIN
  ...
  -- For Monthly: all monthly tasks are first tasks
  -- For Quarterly: tasks with task_period_type = 'monthly' in first month are first tasks
  -- For Yearly: tasks with task_period_type = 'monthly' in first month are first tasks
  ...
  WHERE st.service_id = v_service_id
    ...
    AND (
      -- For monthly recurrence, all monthly tasks are potential first tasks
      (v_work_recurrence = 'monthly' AND st.task_recurrence_type = 'monthly')
      OR
      -- For quarterly/yearly, first tasks must appear in first month (monthly task_period_type)
      (v_work_recurrence IN ('quarterly', 'yearly') AND (st.task_period_type = 'monthly' OR st.task_period_type IS NULL))
    );
...
BEGIN
  ...
  WHILE (v_current_year < v_end_year OR 
    ...
    FOR v_st IN
      SELECT st.id, st.title, st.task_recurrence_type, COALESCE(st.task_period_type, 'monthly') as task_period_type
      FROM service_tasks st
      WHERE st.service_id = v_service_id
        AND st.is_active = TRUE
    LOOP
      ...
      ELSIF v_recurrence_type = 'quarterly' THEN
        ...
        IF v_st.task_period_type = 'monthly' AND v_task_due_date <= p_check_date THEN
          RETURN QUERY SELECT 
            v_st.id,
        ...
        IF v_st.task_period_type = 'quarterly' AND v_month_idx = 3 AND v_task_due_date <= p_check_date THEN
          RETURN QUERY SELECT 
            v_st.id,
            v_st.title,
            v_task_due_date,
      ...
      ELSIF v_recurrence_type = 'yearly' THEN
        ...
        -- Quarterly tasks appear in months 3, 6, 9, 12
        IF v_st.task_period_type = 'quarterly' AND v_month_idx IN (3, 6, 9, 12) AND v_task_due_date <= p_check_date THEN
          RETURN QUERY SELECT 
            v_st.id,
            v_st.title,
            v_task_due_date,
        ...
        IF v_st.task_period_type = 'yearly' AND v_month_idx = 12 AND v_task_due_date <= p_check_date THEN
          RETURN QUERY SELECT 
            v_st.id,
            v_st.title,
            v_task_due_date,
