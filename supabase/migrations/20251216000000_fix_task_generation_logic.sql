/*
  # Fix Task Generation Logic - Strict Date Rules & Custom Recurrence Start
  
  1. Changes:
     - Implements strict rules for creating tasks and periods.
     - Respects `monthly_start_day` and other custom start settings from `works` table.
     - Fixes weekly task generation within monthly periods (avoiding duplicates).
     - Ensure periods align with custom start days if specified.
*/
...
BEGIN
    ...
    WHILE v_iter_date <= v_current_date LOOP
      ...
      LOOP
         ...
         IF LOWER(COALESCE(v_task_record.effective_recurrence, 'daily')) = 'daily' THEN
            ...
            -- RULE CHECK: Period End Passed (Implicit for daily past dates) AND Due Date >= Work Start
  ...
  ELSE 
    ...
    WHILE v_iter_date <= v_current_date LOOP
       -- Adjust v_period_start based on Custom Start Day
       IF v_work_recurrence = 'monthly' THEN
       ...
       LOOP
          ...
          IF v_task_recurrence = v_work_recurrence THEN
             ...
             IF v_period_end <= v_current_date AND v_task_due_date IS NOT NULL AND (v_work_start_date IS NULL OR v_task_due_date >= v_work_start_date) THEN
                v_has_qualifying_task := true; EXIT;
             END IF;
          -- 2. Lower recurrence: Weekly Tasks in Monthly/Quarterly/etc
          ELSIF v_task_recurrence = 'weekly' THEN
             v_task_period_end := v_period_start;
             -- FIND FIRST OCCURRENCE of the Weekday defined in task config (or default Monday)
             ...
             -- Move v_task_period_end to the first matching weekday >= v_period_start
             ...
             WHILE v_task_period_end <= v_period_end LOOP
                 -- Calculate the specific due date for this weekly instance
                 -- v_task_period_end is the "Start of that week" (e.g. Monday)
                 -- The "End of that week" is +6 days. 
                 -- passing start/end to calculate due date (which applies offsets)
                 v_task_due_date := public.calculate_configured_task_due_date(v_task_record, v_task_period_end, (v_task_period_end + 6));
                 
                 -- STRICT CHECK: 
                 -- 1. Due date must be valid (not null).
                 -- 2. Due date must be >= Period Start AND <= Period End. (Strict inclusion in this period)
                 -- 3. Due date must be >= Work Start Date (don't create tasks before work starts)
                 -- 4. Due date <= Current Date (only generate past/due tasks, or pending ones if we want future? Usually we generate up to current)
                 --    Actually, for "auto generate", we usually generate strict "due" periods.
                 --    BUT if the period *itself* is valid (partially past), we might want all tasks in it?
                 --    The outer loop checks `v_iter_date <= v_current_date`. So the period has definitely *started*.
                 --    We should probably include all tasks for this period, even if slightly in future (e.g. due tomorrow), 
                 --    OR stick to "Only generate if due". 
                 --    Let's stick to: If the period is active (iter_date caught up), generate the tasks.
                 ...
                 THEN 
                     v_has_qualifying_task := true; 
                     EXIT; -- Found at least one, so create the period
                 END IF;
                 
                 v_task_period_end := v_task_period_end + INTERVAL '1 week';
             END LOOP;
          -- Other lower recurrences (Daily, etc)
          ELSIF v_task_recurrence = 'daily' THEN
             v_task_period_end := v_period_start;
             while v_task_period_end <= v_period_end LOOP
                v_task_due_date := public.calculate_configured_task_due_date(v_task_record, v_task_period_end, v_task_period_end);
