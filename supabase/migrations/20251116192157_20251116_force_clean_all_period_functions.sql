/*
  # Force clean all period generation functions

  1. Issue
    - Multiple duplicate function versions persisting
    - Old functions with is_active reference still in place
    - Need to completely rebuild the period generation system

  2. Solution
    - Force drop all period-related functions and recreate fresh
    - Ensure no duplicate versions remain
    - Rebuild dependency chain correctly
*/

-- First, show all functions to find duplicates
-- They need to be dropped carefully due to dependencies
...
-- Now recreate clean versions one by one

-- 1. Base function: calculate_next_period_dates
...
-- 2. backfill_missing_periods (depends on calculate_next_period_dates and copy_tasks_to_period)
...
-- 3. auto_generate_next_period_for_work (main function)
...
DECLARE
  ...
  v_period_exists BOOLEAN;
BEGIN
  -- Get work details - explicit column selection, no SELECT *
