/*
  # Fix Invoice Status Changes - Receipt Creation and Ledger Reversal

  ## Issue
  When changing invoice status to 'paid', receipt vouchers are not being created.
  When changing status back to 'sent', receipt vouchers are not being deleted.
  When changing status back to 'draft', invoice ledger entries are not being removed.

  ## Solution
  Complete rewrite of the invoice status change handler to properly:
  1. Status → 'sent': Post invoice to ledger (customer receivable + income)
  2. Status → 'paid': Create receipt voucher with ledger entries
  3. Status 'paid' → 'sent': Delete receipt voucher and its ledger entries
  4. Status 'sent' → 'draft': Delete invoice ledger entries (without voucher_id)
  5. Status 'paid' → 'draft': Delete receipt AND invoice ledger entries
  6. Status → 'cancelled': Delete all related entries

  ## Changes
  - Enhanced receipt voucher creation with better error handling
  - Proper ledger entry reversal when status changes backward
  - Clear separation of invoice ledger entries vs voucher ledger entries
...
-- ============================================================================
-- Complete Invoice Status Change Handler
-- ============================================================================
...
BEGIN
  ...
  IF NEW.status = 'draft' AND v_old_status != 'draft' THEN
    ...
    RAISE NOTICE '  ✓ Deleted % invoice ledger entries', v_receipt_count;
...
COMMENT ON FUNCTION handle_invoice_status_change IS
  'Complete invoice status management with proper ledger reversal:
   ...
   Processing order ensures clean state before posting new entries.
...
BEGIN
  RAISE NOTICE '========================================================================';
  RAISE NOTICE '✓ FIXED INVOICE STATUS CHANGES - RECEIPT AND LEDGER REVERSAL';
  RAISE NOTICE '========================================================================';
  ...
  RAISE NOTICE '✓ Proper ledger reversal on all backward status changes';
