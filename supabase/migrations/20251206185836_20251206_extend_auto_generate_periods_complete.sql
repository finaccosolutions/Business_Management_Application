/*
  # Complete auto_generate_periods_and_tasks with generic due date calculation

  1. New Functions
    - `calculate_task_due_date_in_period`: Generic function for all period types
      Takes period_start and period_end dates instead of just year/month
      Handles all task recurrence types (daily, weekly, monthly, quarterly, half-yearly, yearly)

  2. Enhanced Main Function
    - Added support for yearly work recurrence
    - All period types (daily, weekly, monthly, quarterly, half-yearly, yearly) filter shorter tasks
    - Uses the new generic function for consistent due date calculation across all periods
    - Yearly periods include yearly, half-yearly, quarterly, monthly, weekly, daily tasks
*/
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'calculate_task_due_date_in_period error for task %/%/%: %',
           p_task_id, p_period_start, p_period_end, SQLERRM;
    RETURN NULL;
END;
$$;
...
BEGIN
  ...
  ELSIF v_work_recurrence = 'monthly' THEN
    IF v_work_start_date IS NULL THEN
      v_iter_start := date_trunc('month', v_current_date)::date - INTERVAL '1 month';
    ELSE
      v_iter_start := date_trunc('month', v_work_start_date)::date - INTERVAL '1 month';
    END IF;

    WHILE v_iter_start <= v_current_date LOOP
      v_period_start := v_iter_start;
      v_period_end := (date_trunc('month', v_iter_start)
      ...
      v_iter_start := (v_iter_start + INTERVAL '1 month')::date;
    END LOOP;
        ...
        LOOP
          ...
          IF v_task_recurrence IN ('quarterly', 'monthly', 'weekly', 'daily') THEN
            FOR v_loop_index IN 0..2 LOOP
              v_task_period_end := (v_period_start + (v_loop_index || ' month')::interval)::date;
  ...
  ELSIF v_work_recurrence = 'yearly' THEN
    IF v_work_start_date IS NULL THEN
      v_iter_start := make_date(EXTRACT(YEAR FROM v_current_date)::int - 1, 1, 1);
    ELSE
      v_iter_start := make_date(EXTRACT(YEAR FROM v_work_start_date)::int, 1, 1);
    END IF;

    WHILE v_iter_start <= v_current_date LOOP
      v_period_start := v_iter_start;
      v_period_end := make_date(EXTRACT(YEAR FROM v_iter_start)::int, 12, 31);
      v_period_id := NULL;
      ...
      IF v_period_start <= v_current_date THEN
        ...
        LOOP
          ...
          IF v_task_recurrence IN ('yearly', 'half-yearly', 'quarterly', 'monthly', 'weekly', 'daily') THEN
            FOR v_loop_index IN 0..11 LOOP
              v_task_period_end := (v_period_start + (v_loop_index || ' month')::interval)::date;
              v_task_period_end := (date_trunc('month', v_task_period_end)
                                   + INTERVAL '1 month' - INTERVAL '1 day')::date;
                ...
                IF v_task_period_end <= v_current_date THEN
                  ...
                  IF v_task_due_date IS NOT NULL
                    ...
                    IF v_period_id IS NULL THEN
                      ...
                      IF v_period_id IS NULL THEN
                        v_period_name := 'Y ' || EXTRACT(YEAR FROM v_period_start)::text;
      ...
      v_iter_start := (v_iter_start + INTERVAL '1 year')::date;
    END LOOP;
