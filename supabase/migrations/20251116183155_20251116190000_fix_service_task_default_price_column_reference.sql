/*
  # Fix service_tasks Column Reference Error

  ## Issue
  The copy_tasks_to_period_with_templates function was referencing `v_service_task.default_price`
  which doesn't exist. The service_tasks table only has `estimated_hours`, not `default_price`.
  This caused 400 errors when creating recurring works.

  Error: record "v_service_task" has no field "default_price"

  ## Root Cause
  Line 72 was trying to insert a non-existent field into estimated_hours column.
  The service_tasks table has: id, service_id, title, description, priority, estimated_hours, sort_order, is_active, notes, created_at, updated_at

  ## Fix
  Replace the incorrect field reference with the correct estimated_hours column.
*/
