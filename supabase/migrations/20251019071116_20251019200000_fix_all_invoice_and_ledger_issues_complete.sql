/*
  # Fix All Invoice and Ledger Issues - Complete Solution

  ## Summary
  This migration comprehensively fixes all identified issues with invoice creation,
  ledger mapping, and voucher posting to ledger reports.

  ## Problems Fixed

  ### 1. Recurring Period Auto-Invoice Not Creating
  - Issue: Changing all tasks to completed doesn't create invoice
  - Root Cause: Trigger condition or duplicate check blocking creation
  - Solution: Simplified trigger logic, better duplicate prevention

  ### 2. Ledger Accounts Not Auto-Selecting in Invoice
  - Issue: Income Account and Customer Account fields empty on auto-created invoices
  - Root Cause: Values set in database but not displayed in UI dropdown
  - Solution: Ensure proper ledger mapping in both database and UI

  ### 3. Invoice Number Not Following ID Config Settings
  - Issue: Invoice shows INV-0001 instead of user's configured format (e.g., ABC-00001)
  - Root Cause: generate_next_invoice_number not properly reading invoice config columns
  - Solution: Fix function to read correct column names from company_settings

  ### 4. Invoices/Vouchers Not Reflecting in Ledger Reports
  - Issue: Trial balance and other reports empty even after creating invoices/vouchers
  - Root Cause: Trigger conditions not firing or duplicate checks blocking posting
  - Solution: Simplified posting logic with better duplicate prevention

  ## Changes Made

  1. Fixed generate_next_invoice_number to use correct column names
  2. Ensured auto-invoice functions properly set income_account_id and customer_account_id
  3. Simplified ledger posting triggers to avoid edge cases
  4. Added proper error messages when mappings are missing
*/

-- ============================================================================
-- STEP 1: Fix Invoice Number Generation Function
-- ============================================================================
...
BEGIN
  ...
  -- Build final invoice number with or without suffix
...
COMMENT ON FUNCTION generate_next_invoice_number IS
  'Generates the next invoice number for a user based on company_settings invoice configuration (invoice_prefix, invoice_suffix, invoice_number_width, etc.)';
...
COMMENT ON FUNCTION auto_create_invoice_on_period_completion IS
  'Auto-creates invoices for recurring work periods when all tasks completed. Uses company settings for invoice numbering and validates ledger mappings.';
