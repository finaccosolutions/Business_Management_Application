/*
  # Fix Duplicate get_tasks_to_add_for_period Functions
  
  ## Problem
  Multiple migrations created the same function with identical signatures, causing PostgreSQL to report:
  "function get_tasks_to_add_for_period(uuid, date, date) is not unique"
  
  This prevents any work creation with monthly/quarterly/yearly recurring periods.
  
  ## Solution
  1. Drop all existing function overloads
  2. Recreate a single, definitive version with proper logic
  3. Ensure function can handle all task types and periods
  
  ## Key Features
  - Handles monthly recurring tasks with month suffix names
  - Properly calculates due dates based on task configuration
  - Checks task period expiry before inclusion
*/

-- Drop all duplicate versions
DROP FUNCTION IF EXISTS get_tasks_to_add_for_period(uuid, date, date) CASCADE;

-- Recreate the main function with fixed logic
CREATE FUNCTION get_tasks_to_add_for_period(
  p_service_id UUID,
...
BEGIN
  ...
  FOR v_task IN
    SELECT st.id, st.title, st.description, st.priority, st.estimated_hours, 
           ...
           st.task_recurrence_type, st.due_day_of_month, st.exact_due_date
...
-- Ensure helper functions exist and are unique
