/*
  # Fix Invoice Number Generation Function Call

  ## Problem
  The auto-invoice trigger functions were calling non-existent function
  `generate_invoice_number_from_config(uuid)` when the correct function is
  `generate_next_invoice_number(uuid)`.
...
BEGIN
  ...
  v_invoice_number := generate_next_invoice_number(v_work_record.user_id);
