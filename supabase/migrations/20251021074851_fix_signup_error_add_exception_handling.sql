/*
  # Fix Signup Error - Add Exception Handling to Account Groups Creation

  ## Problem
  User signup fails with "Database error saving new user" because the
  `create_default_account_groups` function doesn't have error handling.
  If any error occurs during account group creation (like constraint violations
  or missing parent references), the entire user signup fails.

  ## Solution
  Add comprehensive error handling to the `create_default_account_groups` function
  to ensure user signup succeeds even if account group creation fails.

  ## Changes
  1. Add EXCEPTION block to catch and log errors
  2. Return NEW even on error to allow user creation to proceed
  3. Add SECURITY DEFINER and search_path settings for security
  4. Maintain the hierarchical structure with proper error recovery

  ## Notes
  - This ensures users can always sign up successfully
  - Account groups can be created manually later if the trigger fails
  - Error is logged as WARNING for debugging
*/
