...
export default function Leads({ isDetailsView, leadId, onNavigate }: LeadsProps) {
  const { user } = useAuth();
    ...
    if (navigationState) {
      try {
        ...
        if (state.itemType === 'lead' && state.shouldShowDetails) {
          onNavigate?.('lead-details', { id: state.id });
  ...
  // If in details view, simplify rendering
  if (isDetailsView && leadId) {
    return (
      <LeadDetails
        leadId={leadId}
              ...
              {(Object.keys(statusConfig) as LeadStatus[]).map((status) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 sm:px-4 py-3 transition-all whitespace-nowrap flex-shrink-0 font-semibold text-sm ${isActive
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-bold min-w-[1.5rem] text-center ${isActive
          ...
          <div className="space-y-2.5">
            {filteredLeads.map((lead) => {
              ...
              return (
                <div
                  key={lead.id}
                  className={`bg-white rounded-lg shadow-sm border-l-4 ${isConverted
                  ...
                  onClick={() => onNavigate?.('lead-details', { id: lead.id })}
                >
                    ...
                    <div className="flex items-start gap-3 justify-between flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onNavigate?.('lead-details', { id: lead.id });
                          }}
