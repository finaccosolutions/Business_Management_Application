...
export default function Vouchers() {
  ...
  const totalPosted = vouchers.filter((v) => v.status === 'posted').reduce((sum, v) => sum + v.total_amount, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-yellow-100 text-sm font-medium">Draft Amount</p>
              <p className="text-3xl font-bold mt-2">₹{totalDraft.toLocaleString('en-IN')}</p>
            </div>
            <div className="p-3 bg-white/20 rounded-lg">
              <FileText className="w-8 h-8" />
            </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-100 text-sm font-medium">Posted Amount</p>
              <p className="text-3xl font-bold mt-2">₹{totalPosted.toLocaleString('en-IN')}</p>
            </div>
            <div className="p-3 bg-white/20 rounded-lg">
              <FileText className="w-8 h-8" />
            </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4 flex-1">
                <div className="p-3 bg-gradient-to-br from-blue-50 to-cyan-100 rounded-lg flex-shrink-0">
                  <FileText className="w-6 h-6 text-blue-600" />
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-bold text-gray-900">{voucher.voucher_number}</h3>
                    <span
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                    ...
                    {voucher.reference_number && (
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{voucher.reference_number}</p>
                      </div>
                    )}
                    <div>
                  ...
                  {voucher.narration && (
                    <p className="text-sm text-gray-600 mt-2">{voucher.narration}</p>
                  )}
                </div>
              </div>
      ...
      {selectedVoucher && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <p className="text-center text-gray-600">Voucher details view coming soon...</p>
            </div>
          </div>
        </div>
      )}
