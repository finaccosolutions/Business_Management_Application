...
export default function ReceiptVoucherModal({ onClose, voucherTypeId, editVoucher }: ReceiptVoucherModalProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (editVoucher && editVoucher.voucher_entries && accounts.length > 0) {
      ...
      const debitEntry = editVoucher.voucher_entries.find(e => e.debit_amount > 0);
      if (debitEntry) {
        setCashBankAccountId(debitEntry.account_id);
      }

      const creditEntriesData = editVoucher.voucher_entries
        .filter(e => e.credit_amount > 0)
        .map(e => ({
      ...
      if (creditEntriesData.length > 0) {
        setCreditEntries(creditEntriesData);
      }
    }
  ...
  const generateVoucherNumber = async () => {
    try {
      const voucherNumber = await generateNextVoucherNumber(user!.id, voucherTypeId, 'receipt');
  ...
  const addCreditEntry = () => {
    setCreditEntries([...creditEntries, { account_id: '', amount: '', narration: '' }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validCreditEntries = creditEntries.filter(
      (entry) => entry.account_id && parseFloat(entry.amount || '0') > 0
    );

    if (validCreditEntries.length === 0) {
      toast.error('Please add at least one credit entry with amount');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
      <div className="fixed top-16 left-0 lg:left-64 right-0 bottom-0 bg-white shadow-2xl flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-green-600 to-emerald-600 flex-shrink-0">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              <FileText size={28} />
              {editVoucher ? 'Edit' : 'Create'} Receipt Voucher
            </h2>
            <p className="text-green-100 text-sm mt-1">Record receipt transactions</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-gray-200">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Credit Ledgers (Customers/Income)</h3>
                <button
                  type="button"
                    ...
                    <div className="grid grid-cols-12 gap-3 items-center">
                      <div className="col-span-12 md:col-span-6">
                        <label className="block text-xs font-medium text-gray-700 mb-1">
                          Credit Ledger (Customer/Income) *
                        </label>
                        <select
                          value={entry.account_id}
                          onChange={(e) => updateCreditEntry(index, 'account_id', e.target.value)}
                      ...
                      <div className="col-span-12 md:col-span-10">
                        ...
                        <input
                          ...
                          onChange={(e) => updateCreditEntry(index, 'narration', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500"
            ...
            <div className="bg-gradient-to-r from-gray-50 to-green-50 rounded-xl p-6 border border-gray-200">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-base">
                  <span className="text-gray-700 font-medium">Total Receipt Amount:</span>
                ...
                <div className="p-3 bg-white rounded-lg border border-green-200">
                  ...
                  <p className="text-xs text-green-600 mt-1">
                    Will be DEBITED with ₹{totalAmount.toFixed(2)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          >
            {editVoucher ? 'Update' : 'Create'} Receipt Voucher
          </button>
        </div>
      </div>
    </div>
