...
export default function ServiceDetails({ serviceId, onBack, onUpdate, onNavigateToCustomer, onNavigateToWork }: ServiceDetailsProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] bg-white shadow-sm overflow-hidden">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 font-medium rounded-t-lg transition-all whitespace-nowrap ${activeTab === tab.id
      ...
      <div className="flex-1 overflow-y-auto p-6">
                ...
                {customerServices.map((cs) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium border ${statusColors[cs.status] || statusColors.inactive
                          }`}
                      >
                        {cs.status}
                      </span>
        ...
        {/* ... works tab logic remains same, just adjusted classNames ... */}
        {activeTab === 'works' && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">
                ...
                {works.map((work) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium ${work.status === 'completed'
                ...
                {customerServices
                  ...
                  .map((cs) => (
                    <div key={cs.id} className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded ${cs.status === 'active'
                          ? 'bg-green-100 text-green-700'
        ...
        {/* ... tasks and documents and activity tabs ... */}
        {activeTab === 'tasks' && (
          <div className="space-y-4">
            {service.is_recurring && (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${task.priority === 'urgent'
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex flex-col gap-1 border-r pr-2">
                          ...
                          <button
                            ...
                            className={`p-1 rounded transition-colors ${index === serviceTasks.length - 1
                              ? 'text-gray-300 cursor-not-allowed'
                              : 'text-gray-600 hover:bg-gray-100'
                              }`}
                            title="Move down"
      ...
      {showDocumentModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full">
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-cyan-600">
              <h3 className="text-xl font-bold text-white">{editingDocument ? 'Edit Document' : 'Add Document'}</h3>
            </div>
            <form onSubmit={async (e) => {
              ...
              try {
                if (editingDocument) {
                  const { error } = await supabase.from('service_documents').update(docData).eq('id', editingDocument.id);
                  if (error) throw error;
              ...
              <div>
                ...
                <input type="text" name="name" required defaultValue={editingDocument?.name || ''} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Description</label>
                <textarea name="description" defaultValue={editingDocument?.description || ''} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" rows={2} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Category</label>
                <select name="category" defaultValue={editingDocument?.category || 'general'} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" name="is_required" id="is_required" defaultChecked={editingDocument?.is_required || false} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                <label htmlFor="is_required" className="text-sm font-medium text-gray-700">Required</label>
              </div>
              <div className="flex space-x-3 pt-4">
                <button type="button" onClick={() => { setShowDocumentModal(false); setEditingDocument(null); }} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">{editingDocument ? 'Update' : 'Add'}</button>
              </div>
            </form>
          </div>
        </div>
