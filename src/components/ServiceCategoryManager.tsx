...
import { X, Plus, Edit2, Trash2, Folder, FolderTree, Save, XCircle, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ServiceCategoryManager({ onClose, onCategoryUpdate }: ServiceCategoryManagerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-blue-600 via-cyan-600 to-blue-700 p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">
              Organize your services with categories and subcategories for better tracking
            </p>
          </div>
          <button
            onClick={onClose}
                    ...
                    {PRESET_COLORS.map(color => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-lg transition-all ${
                          newCategory.color === color ? 'ring-2 ring-offset-2 ring-blue-500 scale-110' : 'hover:scale-105'
                        }`}
                                ...
                                {PRESET_COLORS.map(color => (
                                  <button
                                    ...
                                    className={`w-6 h-6 rounded ${
                                      editingCategory.color === color ? 'ring-2 ring-blue-500' : ''
                                    }`}
                                    style={{ backgroundColor: color }}
                                  />
                                ))}
